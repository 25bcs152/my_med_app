  ...
  void initState() {
    ...
    _medicine1Stream = FirebaseFirestore.instance
        ...
        .map((snapshot) {
          ...
          if (data == null || !data.containsKey('items')) return <Medicine1>[];
    ...
    _medicine2Stream = FirebaseFirestore.instance
        ...
        .map((snapshot) {
          final data = snapshot.data();
          if (data == null || !data.containsKey('items')) return <Medicine2>[];
          final List<dynamic> items = data['items'];
          return items.map((item) => Medicine2.fromJson(item)).toList();
        });

    // SYNC: Listen to streams to keep 'My Medicines' updated
    _medicine1Stream.listen((list) {
      if (mounted) context.read<AppState>().syncMedicines(list);
    });
    _medicine2Stream.listen((list) {
      if (mounted) context.read<AppState>().syncMedicines(list);
    });
  }
        ...
        Expanded(
          child: StreamBuilder<List<T>>(
            ...
            builder: (context, snapshot) {
              ...
              // Optimistic UI or empty while loading initial single doc
