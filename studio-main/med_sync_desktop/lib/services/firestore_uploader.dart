...
class FirestoreUploader {
  ...
  Future<void> init() async {
    ...
    if (saJson['project_id'] == null) {
        throw Exception('Invalid Service Account JSON: "project_id" field is missing.');
    }
  ...
  Future<void> uploadBatch(String collectionName, List<Map<String, dynamic>> rows, Function(String) log) async {
    ...
    for (var i = 0; i < rows.length; i += batchSize) {
        ...
        for (var row in chunk) {
            ...
            // Ensure 'id' field exists in the document body as well, for robustness
            data['id'] = docId;
            data.remove('_id'); 
